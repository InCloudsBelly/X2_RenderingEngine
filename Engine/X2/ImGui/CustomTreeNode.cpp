...
bool TreeNodeWithIcon(X2::Ref<X2::VulkanTexture2D> icon, ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end, ImColor iconTint/* = IM_COL32_WHITE*/)
{
    ...
    if (display_frame)
    {
        ...
        if (icon)
        {
            ...
            X2::UI::ShiftCursorY(-frame_height + pad);
            X2::UI::ShiftCursorX(arrowWidth);
            X2::UI::Image(icon, { frame_height - pad * 2.0f, frame_height - pad * 2.0f }, ImVec2(0, 0), ImVec2(1, 1), iconTint /*selected ? colourDark : tintFloat*/);
    ...
    else
    {
        ...
        if (hovered || selected)
        {
            //if (held && hovered) X2_CORE_WARN("held && hovered");
            //if(hovered && !selected && !held && !pressed && !toggled) X2_CORE_WARN("hovered && !selected && !held");
            //else if(!selected) X2_CORE_WARN("ImGuiCol_Header");

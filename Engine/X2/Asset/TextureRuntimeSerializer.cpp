...
namespace X2 {
	...
	uint64_t TextureRuntimeSerializer::SerializeToFile(Ref<VulkanTextureCube> textureCube, FileStreamWriter& stream)
	{
		...
		for (int i = 0; i < 1; i++)
		{
			...
			X2_CORE_WARN("Waiting for env map...");
		}
	...
	Ref<VulkanTextureCube> TextureRuntimeSerializer::DeserializeTextureCube(FileStreamReader& stream)
	{
		struct TextureCubeMetadata
		{
			uint32_t Width, Height;
	...
	Ref<VulkanTexture2D> TextureRuntimeSerializer::DeserializeTexture2D(FileStreamReader& stream)
	{
		Texture2DMetadata metadata;

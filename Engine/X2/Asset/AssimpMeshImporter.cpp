...
//#include "X2/Animation/AnimationImporterAssimp.h"
#include "X2/Utilities/AssimpLogStream.h"
...
#define X2_MESH_ERROR(...)
	...
	namespace Utils {
		...
		{
			X2_MESH_LOG("{0:^{1}}{2} {{", "", depth * 3, node->mName.C_Str());
			++depth;
			...
			X2_MESH_LOG("{0:^{1}}translation: ({2:6.2f}, {3:6.2f}, {4:6.2f})", "", depth * 3, translation.x, translation.y, translation.z);
			X2_MESH_LOG("{0:^{1}}rotation:    ({2:6.2f}, {3:6.2f}, {4:6.2f})", "", depth * 3, rotation.x, rotation.y, rotation.z);
			X2_MESH_LOG("{0:^{1}}scale:       ({2:6.2f}, {3:6.2f}, {4:6.2f})", "", depth * 3, scale.x, scale.y, scale.z);
			for (uint32_t i = 0; i < node->mNumChildren; ++i)
			{
				PrintNode(node->mChildren[i], depth);
			}
			--depth;
			X2_MESH_LOG("{0:^{1}}}}", "", depth * 3);
		}
#endif
	...
	Ref<MeshSource> AssimpMeshImporter::ImportToMeshSource()
	{
		...
		{
			X2_CORE_ERROR_TAG("Mesh", "Failed to load mesh file: {0}", m_Path.string());
			meshSource->SetFlag(AssetFlag::Invalid);
			return nullptr;
		}

		/*meshSource->m_Skeleton = AnimationImporterAssimp::ImportSkeleton(scene);
		X2_CORE_INFO_TAG("Animation", "Skeleton {0} found in mesh file '{1}'", meshSource->HasSkeleton() ? "" : "not", m_Path.string());
		...
		if (scene->HasMeshes())
		{
			...
			for (unsigned m = 0; m < scene->mNumMeshes; m++)
			{
				...
				X2_CORE_ASSERT(mesh->HasPositions(), "Meshes require positions.");
				X2_CORE_ASSERT(mesh->HasNormals(), "Meshes require normals.");
				...
				{
					X2_CORE_ASSERT(mesh->mFaces[i].mNumIndices == 3, "Must have 3 indices.");
...
#if MESH_DEBUG_LOG
			X2_CORE_INFO_TAG("Mesh", "Traversing nodes for scene '{0}'", filename);
		...
		//				{
		//					X2_CORE_ERROR_TAG("Animation", "Could not find mesh bone '{}' in skeleton!", bone->mName.C_Str());
		//				}
		...
		// Materials
		Ref<VulkanTexture2D> whiteTexture = Renderer::GetWhiteTexture();
		Ref<VulkanTexture2D> blackTexture = Renderer::GetBlackTexture();

		if (scene->HasMaterials())
		{
			X2_MESH_LOG("---- Materials - {0} ----", filename);
			...
			for (uint32_t i = 0; i < scene->mNumMaterials; i++)
			{
				...
				X2_MESH_LOG("    TextureCount = {0}", textureCount);
				...
				X2_MESH_LOG("    COLOR = {0}, {1}, {2}", aiColor.r, aiColor.g, aiColor.b);
				X2_MESH_LOG("    ROUGHNESS = {0}", roughness);
				X2_MESH_LOG("    METALNESS = {0}", metalness);
				...
				if (hasAlbedoMap)
				{
					...
					if (auto aiTexEmbedded = scene->GetEmbeddedTexture(aiTexPath.C_Str()))
					{
						...
						textureHandle = AssetManager::CreateMemoryOnlyRendererAsset<VulkanTexture2D>(spec, Buffer(aiTexEmbedded->pcData, 1));
					}
					else
					{
						// TODO: Temp - this should be handled by X2's filesystem
						auto parentPath = m_Path.parent_path();
						parentPath /= std::string(aiTexPath.data);
						std::string texturePath = parentPath.string();
						X2_MESH_LOG("    Albedo map path = {0}", texturePath);
						textureHandle = AssetManager::CreateMemoryOnlyRendererAsset<VulkanTexture2D>(spec, texturePath);
					}

					Ref<VulkanTexture2D> texture = AssetManager::GetAsset<VulkanTexture2D>(textureHandle);
					...
					{
						X2_CORE_ERROR_TAG("Mesh", "Could not load texture: {0}", aiTexPath.C_Str());
						fallback = true;
					}
				}
				...
				if (hasNormalMap)
				{
					...
					else
					{
						...
						parentPath /= std::string(aiTexPath.data);
						std::string texturePath = parentPath.string();
						X2_MESH_LOG("    Normal map path = {0}", texturePath);
						textureHandle = AssetManager::CreateMemoryOnlyRendererAsset<VulkanTexture2D>(spec, texturePath);
					}
				...
				if (fallback)
				{
					X2_MESH_LOG("    No normal map");
					mi->Set("u_NormalTexture", whiteTexture);
					mi->Set("u_MaterialUniforms.UseNormalMap", false);
				}

				// MetallicRoughness map
				bool hasMetallicRoughnessMap = aiMaterial->GetTexture(AI_MATKEY_GLTF_PBRMETALLICROUGHNESS_METALLICROUGHNESS_TEXTURE, &aiTexPath) == AI_SUCCESS;
				fallback = !hasMetallicRoughnessMap;
				if (hasMetallicRoughnessMap)
				{
					...
					else
					{
						...
						parentPath /= std::string(aiTexPath.data);
						std::string texturePath = parentPath.string();
						X2_MESH_LOG("    Roughness map path = {0}", texturePath);
						textureHandle = AssetManager::CreateMemoryOnlyRendererAsset<VulkanTexture2D>(spec, texturePath);
					}
					...
					if (texture && texture->Loaded())
					{
						mi->Set("u_MetallicRoughnessTexture", texture);
				...
				if (fallback)
				{
					X2_MESH_LOG("    No roughness map");
					mi->Set("u_MetallicRoughnessTexture", whiteTexture);
					mi->Set("u_MaterialUniforms.Roughness", roughness);
					mi->Set("u_MaterialUniforms.Metalness", metalness);
				}

				// Emission map
				bool haEmissionMap = aiMaterial->GetTexture(aiTextureType_EMISSIVE,0, &aiTexPath) == AI_SUCCESS;
				fallback = !haEmissionMap;
				if (haEmissionMap)
				{
					AssetHandle textureHandle = 0;
					TextureSpecification spec;
					...
					else
					{
						...
						parentPath /= std::string(aiTexPath.data);
						std::string texturePath = parentPath.string();
						X2_MESH_LOG(" Emission map path = {0}", texturePath);
						textureHandle = AssetManager::CreateMemoryOnlyRendererAsset<VulkanTexture2D>(spec, texturePath);
					}
					...
					if (texture && texture->Loaded())
					{
						mi->Set("u_EmissionTexture", texture);
						mi->Set("u_MaterialUniforms.Emission", emission);
				...
				if (fallback)
				{
					X2_MESH_LOG("    No Emission map");
					mi->Set("u_EmissionTexture", blackTexture);
					mi->Set("u_MaterialUniforms.Emission", emission);
				}
				...
				if (aiMaterial->Get("$raw.ReflectionFactor|file", aiPTI_String, 0, aiTexPath) == AI_SUCCESS)
				{
					...
					auto texture = VulkanTexture2D::Create(texturePath);
					if (texture->Loaded())
					{
						X2_MESH_LOG("    Metalness map path = {0}", texturePath);
						mi->Set("u_MetalnessTexture", texture);
						mi->Set("u_MetalnessTexToggle", 1.0f);
					}
					else
					{
						X2_CORE_ERROR_TAG("Mesh", "Could not load texture: {0}", texturePath);
					}
				}
				else
				{
					X2_MESH_LOG("    No metalness texture");
...
//					X2_MESH_LOG("VulkanMaterial Property:");
//					X2_MESH_LOG("  Name = {0}", prop->mKey.data);
//					// X2_MESH_LOG("  Type = {0}", prop->mType);
//					// X2_MESH_LOG("  Size = {0}", prop->mDataLength);
//					float data = *(float*)prop->mData;
//					X2_MESH_LOG("  Value = {0}", data);
//
//					switch (prop->mSemantic)
//					{
//					case aiTextureType_NONE:
//						X2_MESH_LOG("  Semantic = aiTextureType_NONE");
//						break;
//					case aiTextureType_DIFFUSE:
//						X2_MESH_LOG("  Semantic = aiTextureType_DIFFUSE");
//						break;
//					case aiTextureType_SPECULAR:
//						X2_MESH_LOG("  Semantic = aiTextureType_SPECULAR");
//						break;
//					case aiTextureType_AMBIENT:
//						X2_MESH_LOG("  Semantic = aiTextureType_AMBIENT");
//						break;
//					case aiTextureType_EMISSIVE:
//						X2_MESH_LOG("  Semantic = aiTextureType_EMISSIVE");
//						break;
//					case aiTextureType_HEIGHT:
//						X2_MESH_LOG("  Semantic = aiTextureType_HEIGHT");
//						break;
//					case aiTextureType_NORMALS:
//						X2_MESH_LOG("  Semantic = aiTextureType_NORMALS");
//						break;
//					case aiTextureType_SHININESS:
//						X2_MESH_LOG("  Semantic = aiTextureType_SHININESS");
//						break;
//					case aiTextureType_OPACITY:
//						X2_MESH_LOG("  Semantic = aiTextureType_OPACITY");
//						break;
//					case aiTextureType_DISPLACEMENT:
//						X2_MESH_LOG("  Semantic = aiTextureType_DISPLACEMENT");
//						break;
//					case aiTextureType_LIGHTMAP:
//						X2_MESH_LOG("  Semantic = aiTextureType_LIGHTMAP");
//						break;
//					case aiTextureType_REFLECTION:
//						X2_MESH_LOG("  Semantic = aiTextureType_REFLECTION");
//						break;
//					case aiTextureType_UNKNOWN:
//						X2_MESH_LOG("  Semantic = aiTextureType_UNKNOWN");
//						break;
//					}
//#endif
//
...
//							else
//							{
//								X2_CORE_ERROR_TAG("Mesh", "    Could not load texture: {0}", str);
//							}
//							break;
//						}
//					}
...
//				}
	...
	//	skeleton = AnimationImporterAssimp::ImportSkeleton(scene);
	//	return true;
	//}

	//bool AssimpMeshImporter::ImportAnimations(const uint32_t animationIndex, const Skeleton& skeleton, std::vector<Scope<Animation>>& animations)
	//{
	//	Assimp::Importer importer;
	//	importer.SetPropertyBool(AI_CONFIG_IMPORT_FBX_PRESERVE_PIVOTS, false);
	...
	//	for (const auto& animationName : animationNames)
	//	{
	//		animations.emplace_back(AnimationImporterAssimp::ImportAnimation(scene, animationName, skeleton));
	//	}
	...
	void AssimpMeshImporter::TraverseNodes(Ref<MeshSource> meshSource, void* assimpNode, uint32_t nodeIndex, const glm::mat4& parentTransform, uint32_t level)
	{
		...
		// X2_MESH_LOG("{0} {1}", LevelToSpaces(level), node->mName.C_Str());

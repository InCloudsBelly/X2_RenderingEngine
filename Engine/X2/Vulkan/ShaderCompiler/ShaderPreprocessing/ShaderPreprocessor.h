...
namespace std {
	template<>
	struct hash<X2::IncludeData>
	{
		size_t operator()(const X2::IncludeData& data) const noexcept
		{
...
namespace X2 {
		...
		while (startOfShaderStage != std::string::npos)
		{
			...
			if (tokens[index] == "#")
			{
				...
				if (tokens[index] == "pragma")
				{
					...
					{
						X2_CORE_VERIFY(tokens[++index] == ":", "Stage pragma is invalid");

						// Skipped ':'
						const std::string_view stage(tokens[++index]);
						X2_CORE_VERIFY(stage == "vert" || stage == "frag" || stage == "comp", "Invalid shader type specified");
						...
						if (isGuarded && alreadyIncluded)
							...
							X2_CORE_WARN("\"{}\" Header does not contain a header guard (#pragma once).", fullPath);
				...
				{
					++index;
					if (tokens[index].rfind("__X2_", 0) == 0) // X2 special macros start with "__X2_"
					{
				...
				else if (tokens[index] == "if" || tokens[index] == "define") // Consider "#if defined()" too?
				{
					...
					{
						if (tokens[i].rfind("__X2_", 0) == 0) // X2 special macros start with "__X2_"
						{
	...
	std::map<VkShaderStageFlagBits, std::string> ShaderPreprocessor::PreprocessShader(const std::string& source, std::unordered_set<std::string>& specialMacros)
	{
		...
		X2_CORE_ASSERT(newSource.size(), "Shader is empty!");
		...
		if constexpr (Lang == ShaderUtils::SourceLang::GLSL)
		{
			...
			X2_CORE_VERIFY(tokens.size() >= 3 && tokens[1] == "version", "Invalid #version encountered or #version is NOT encounted first.");
		...
		X2_CORE_VERIFY(stagePositions.size(), "Could not pre-process shader! There are no known stages defined in file.");

...
namespace X2 {
	...
	namespace Utils {
		static VkPrimitiveTopology GetVulkanTopology(PrimitiveTopology topology)
		{
			...
			X2_CORE_ASSERT(false, "Unknown toplogy");
		...
		static VkCompareOp GetVulkanCompareOperator(const DepthCompareOperator compareOp)
		{
			...
			X2_CORE_ASSERT(false, "Unknown Operator");
		...
		Renderer::Submit([instance]() mutable
			{
				...
				if (!shaderDescriptorSets.empty())
				{
					...
					X2_CORE_WARN("VulkanPipeline - Updating {0} descriptor sets", writeDescriptors.size());
	...
	void VulkanPipeline::RT_SetUniformBuffer(Ref<VulkanUniformBuffer> uniformBuffer, uint32_t binding, uint32_t set)
	{
		...
		X2_CORE_WARN("VulkanPipeline - Updating descriptor set (VulkanPipeline::SetUniformBuffer)");
		VkDevice device = VulkanContext::GetCurrentDevice()->GetVulkanDevice();

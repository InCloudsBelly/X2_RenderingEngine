...
namespace X2 {
		...
		: m_Specification(specification)
	{
		...
		X2_CORE_ASSERT(specification.Attachments.Attachments.size());
	...
	{
		// X2_CORE_TRACE("VulkanFramebuffer::RT_Invalidate ({})", m_Specification.DebugName);
		...
		for (auto attachmentSpec : m_Specification.Attachments.Attachments)
		{
			if (Utils::IsDepthFormat(attachmentSpec.Format))
			{
				...
				{
					Ref<VulkanImage2D> existingImage = m_Specification.ExistingImages.at(attachmentIndex);
					X2_CORE_ASSERT(Utils::IsDepthFormat(existingImage->GetSpecification().Format), "Trying to attach non-depth image as depth attachment");
					m_DepthAttachmentImage = existingImage;
				}
				else
				{
					Ref<VulkanImage2D> depthAttachmentImage = m_DepthAttachmentImage;
			...
			{
				//X2_CORE_ASSERT(!m_Specification.ExistingImage, "Not supported for color attachments");
				...
				else
				{
					...
					{
						Ref<VulkanImage2D> image = m_AttachmentImages[attachmentIndex];
				...
				attachmentDescription.stencilLoadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE;
				attachmentDescription.stencilStoreOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;
				attachmentDescription.initialLayout = m_Specification.ClearColorOnLoad ? VK_IMAGE_LAYOUT_UNDEFINED : VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
		...
		if (m_DepthAttachmentImage)
		{
			...
			X2_CORE_ASSERT(attachments.back());
		}

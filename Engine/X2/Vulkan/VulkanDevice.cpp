...
namespace X2
{
	...
	VulkanPhysicalDevice::VulkanPhysicalDevice()
	{
		...
		{
			X2_CORE_TRACE_TAG("Renderer", "Could not find discrete GPU.");
			selectedPhysicalDevice = physicalDevices.back();
		}

		X2_CORE_ASSERT(selectedPhysicalDevice, "Could not find any physical devices!");
		...
		X2_CORE_ASSERT(queueFamilyCount > 0, "");
		...
		if (extCount > 0)
		{
			...
			{
				X2_CORE_TRACE_TAG("Renderer", "Selected physical device has {0} extensions", extensions.size());
				for (const auto& ext : extensions)
				{
					m_SupportedExtensions.emplace(ext.extensionName);
					X2_CORE_TRACE_TAG("Renderer", "  {0}", ext.extensionName);
				}
			}
		}
	...
	uint32_t VulkanPhysicalDevice::GetMemoryTypeIndex(uint32_t typeBits, VkMemoryPropertyFlags properties) const
	{
		...
		X2_CORE_ASSERT(false, "Could not find a suitable memory type!");
		return UINT32_MAX;
	}
		...
		: m_PhysicalDevice(physicalDevice), m_EnabledFeatures(enabledFeatures)
	{
		...
		X2_CORE_ASSERT(m_PhysicalDevice->IsExtensionSupported(VK_KHR_SWAPCHAIN_EXTENSION_NAME));
	...
	Ref<VulkanCommandPool> VulkanDevice::GetThreadLocalCommandPool()
	{
		...
		X2_CORE_VERIFY(m_CommandPools.find(threadID) != m_CommandPools.end());
	...
	void VulkanCommandPool::FlushCommandBuffer(VkCommandBuffer commandBuffer, VkQueue queue)
	{
		...
		X2_CORE_ASSERT(commandBuffer != VK_NULL_HANDLE);

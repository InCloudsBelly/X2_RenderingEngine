...
namespace X2::Utils {
	...
	//		vkGetQueueCheckpointDataNV(::X2::VulkanContext::GetCurrentDevice()->GetGraphicsQueue(), &retrievedCount, data);
	//		X2_CORE_ERROR("RetrieveDiagnosticCheckpoints (Graphics Queue):");
	//		for (uint32_t i = 0; i < retrievedCount; i++)
	//		{
	//			VulkanCheckpointData* checkpoint = (VulkanCheckpointData*)data[i].pCheckpointMarker;
	//			X2_CORE_ERROR("Checkpoint: {0} (stage: {1})", checkpoint->Data, StageToString(data[i].stage));
	//		}
	//	}
	//	{
	//		const uint32_t checkpointCount = 4;
	...
	//		uint32_t retrievedCount = checkpointCount;
	//		vkGetQueueCheckpointDataNV(::X2::VulkanContext::GetCurrentDevice()->GetComputeQueue(), &retrievedCount, data);
	//		X2_CORE_ERROR("RetrieveDiagnosticCheckpoints (Compute Queue):");
	//		for (uint32_t i = 0; i < retrievedCount; i++)
	//		{
	//			VulkanCheckpointData* checkpoint = (VulkanCheckpointData*)data[i].pCheckpointMarker;
	//			X2_CORE_ERROR("Checkpoint: {0} (stage: {1})", checkpoint->Data, StageToString(data[i].stage));

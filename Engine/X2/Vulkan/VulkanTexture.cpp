...
#include "X2/Asset/TextureImporter.h"
...
namespace X2 {
	...
	namespace Utils {
		...
		static VkSamplerAddressMode VulkanSamplerWrap(TextureWrap wrap)
		{
			...
			X2_CORE_ASSERT(false, "Unknown wrap mode");
		...
		static VkFilter VulkanSamplerFilter(TextureFilter filter)
		{
			...
			X2_CORE_ASSERT(false, "Unknown filter");
			return (VkFilter)0;
		}
		...
		: m_Path(filepath), m_Specification(specification)
	{
		...
		if (!m_ImageData)
		{
			...
			m_ImageData = TextureImporter::ToBufferFromFile(std::string(PROJECT_ROOT) + "Resources/Textures/ErrorTexture.png", m_Specification.Format, m_Specification.Width, m_Specification.Height);
		}
	...
	void VulkanTexture2D::Invalidate()
	{
		...
		if (m_ImageData)
		{
			...
			X2_CORE_ASSERT(m_ImageData.Data);
		...
		else
		{
			...
			Utils::SetImageLayout(transitionCommandBuffer, info.Image, VK_IMAGE_LAYOUT_UNDEFINED, image->GetDescriptorInfo().imageLayout, subresourceRange);
	...
	{
		X2_CORE_VERIFY(buffer.Size == m_GPUAllocationSize);

...
namespace X2 {
	...
	{
		// Print Info
		X2_CORE_TRACE("Serializing AssetPack to {}", path.string());
		X2_CORE_TRACE("  {} scenes", file.Index.Scenes.size());
		uint32_t assetCount = 0;
		for (const auto& [sceneHandle, sceneInfo] : file.Index.Scenes)
			assetCount += sceneInfo.Assets.size();
		X2_CORE_TRACE("  {} assets (including duplicates)", assetCount);
		...
		X2_CORE_TRACE("Serialized {} assets into AssetPack", serializedAssets.size());
	...
	{
		// Print Info
		X2_CORE_TRACE("Deserializing AssetPack from {}", path.string());
		...
		bool validHeader = memcmp(file.Header.HEADER, "X2AP", 4) == 0;
		...
		X2_CORE_TRACE("Deserialized index with {} scenes from AssetPack", sceneCount);
		return true;
	}

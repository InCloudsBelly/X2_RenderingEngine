...
namespace YAML {
	...
	template<>
	struct convert<X2::AssetHandle>
	{
		static Node encode(const X2::AssetHandle& rhs)
		{
			Node node;
		...
		static bool decode(const Node& node, X2::AssetHandle& rhs)
		{
	...
	private:
		...
		static Node encodeObject(const choc::value::ValueView& object)
		{
			...
			const bool isUUID = object.isObjectWithClassName("UUID") && object.hasObjectMember("Value");
			if (isUUID)
			{
				const auto& value = object["Value"];
				X2_CORE_ASSERT(value.isInt64());
	...
	private:
		...
		static bool decodeObject(const Node& node, choc::value::Value& rhs)
		{
			...
			// So that in the readable YAML it is shown as the correct uint64_t,
			// while for choc::Value it's casted to int64_t
			const bool isUUID = rhs.isObjectWithClassName("UUID") && node["Value"];
			if (isUUID)
			{

...
#define MAX_INPUT_BUFFER_LENGTH 128
	...
	public:
		ContentBrowserItem(ItemType type, AssetHandle id, const std::string& name, const Ref<VulkanTexture2D>& icon);
		...
		const Ref<VulkanTexture2D>& GetIcon() const { return m_Icon; }
	...
	{
	public:
		ContentBrowserAsset(const AssetMetadata& assetInfo, const Ref<VulkanTexture2D>& icon);

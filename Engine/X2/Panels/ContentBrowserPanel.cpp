...
namespace X2 {
		...
		: m_Project(nullptr)
	{
		...
		m_AssetIconMap[".xmaterial"] = EditorResources::MaterialFileIcon;
		m_AssetIconMap[".xscene"] = EditorResources::SceneFileIcon;
		m_AssetIconMap[".xprefab"] = EditorResources::PrefabFileIcon;
		m_AssetIconMap[".hanim"] = EditorResources::AnimationFileIcon;
		m_AssetIconMap[".hanimc"] = EditorResources::AnimationControllerFileIcon;
		m_AssetIconMap[".xmesh"] = EditorResources::MeshFileIcon;
		m_AssetIconMap[".xsmesh"] = EditorResources::StaticMeshFileIcon;
		m_AssetIconMap[".xmc"] = EditorResources::MeshColliderFileIcon;
		m_AssetIconMap[".xpm"] = EditorResources::PhysicsMaterialFileIcon;
		m_AssetIconMap[".xskel"] = EditorResources::SkeletonFileIcon;
						...
						if (ImGui::BeginPopupContextWindow(0, 1, false))
						{
							if (ImGui::BeginMenu("New"))
							{
								...
								/*if (ImGui::MenuItem("Animation Controller"))
									CreateAsset<AnimationController>("New Animation Controller.hanimc");*/
				...
				if (ImGui::BeginDragDropTarget())
				{
					...
					if (data)
					{
						...
						for (size_t i = 0; i < count; i++)
						{
							...
							if (!entity)
							{
								X2_CORE_ERROR_TAG("ContentBrowser", "Failed to find Entity with ID {0} in current Scene context!", entityID);
								continue;
							}
	...
	void ContentBrowserPanel::OnProjectChanged(const Ref<Project>& project)
	{
		...
		Project::GetEditorAssetManager()->SetAssetChangeCallback(X2_BIND_EVENT_FN(ContentBrowserPanel::OnFileSystemChanged));
	...
	namespace UI {
		...
		static bool TreeNode(const std::string& id, const std::string& label, ImGuiTreeNodeFlags flags = 0, const Ref<VulkanTexture2D>& icon = nullptr)
		{
	...
	void ContentBrowserPanel::RenderDirectoryHierarchy(Ref<DirectoryInfo>& directory)
	{
		...
		{
			fillWithColour(Colours::Theme::selectionMuted);
		}
	...
	void ContentBrowserPanel::RenderTopBar(float height)
	{
		...
		ImGui::BeginHorizontal("##top_bar", ImGui::GetWindowSize());
		{
			...
			// Navigation buttons
			{
				...
				auto contenBrowserButton = [height](const char* labelId, const Ref<VulkanTexture2D>& icon)
				{
					const ImU32 buttonCol = Colours::Theme::backgroundDark;
					const ImU32 buttonColP = UI::ColourWithMultipliedValue(Colours::Theme::backgroundDark, 0.8f);
					...
					UI::DrawButtonImage(icon, Colours::Theme::textDarker,
						UI::ColourWithMultipliedValue(Colours::Theme::textDarker, 1.2f),
						UI::ColourWithMultipliedValue(Colours::Theme::textDarker, 0.8f),

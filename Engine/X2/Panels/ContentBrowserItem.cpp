...
// TEMP
#include "X2/Asset/MeshRuntimeSerializer.h"
...
namespace X2 {
	...
	ContentBrowserItem::ContentBrowserItem(ItemType type, AssetHandle id, const std::string& name, const Ref<VulkanTexture2D>& icon)
	...
	CBItemActionResult ContentBrowserItem::OnRender()
	{
		...
		if (m_Type != ItemType::Directory)
		{
			...
			// Draw background
			drawList->AddRectFilled(topLeft, thumbBottomRight, Colours::Theme::backgroundDark);
			drawList->AddRectFilled(infoTopLeft, bottomRight, Colours::Theme::groupHeader, 6.0f, ImDrawFlags_RoundCornersBottom);
		}
		else if (ImGui::ItemHoverable(ImRect(topLeft, bottomRight), ImGui::GetID(&m_ID)) || isSelected)
		{
			...
			auto* drawList = ImGui::GetWindowDrawList();
			drawList->AddRectFilled(topLeft, bottomRight, Colours::Theme::groupHeader, 6.0f);
		}
		...
		if (isSelected || ImGui::IsItemHovered())
		{
			...
			if (isSelected)
			{
				...
				ImColor colTransition = UI::ColourWithMultipliedValue(Colours::Theme::selection, 0.8f);

				drawList->AddRect(itemRect.Min, itemRect.Max,
					mouseDown ? colTransition : Colours::Theme::selection, 6.0f,
	...
	void ContentBrowserDirectory::OnRenamed(const std::string& newName)
	{
		...
		{
			X2_CORE_ERROR("Couldn't rename {0} to {1}!", m_DirectoryInfo->FilePath.filename().string(), newName);
		}
	}
	...
	void ContentBrowserDirectory::Delete()
	{
		...
		if (!deleted)
		{
			X2_CORE_ERROR("Failed to delete folder {0}", m_DirectoryInfo->FilePath);
			return;
		}
	...
	ContentBrowserAsset::ContentBrowserAsset(const AssetMetadata& assetInfo, const Ref<VulkanTexture2D>& icon)
	...
	void ContentBrowserAsset::Delete()
	{
		...
		if (!deleted)
		{
			X2_CORE_ERROR("Couldn't delete {0}", m_AssetInfo.FilePath);
			return;
		}
	...
	bool ContentBrowserAsset::Move(const std::filesystem::path& destination)
	{
		...
		if (!wasMoved)
		{
			X2_CORE_ERROR("Couldn't move {0} to {1}", m_AssetInfo.FilePath, destination);
			return false;
		}
	...
	void ContentBrowserAsset::OnRenamed(const std::string& newName)
	{
		...
		{
			X2_CORE_ERROR("Couldn't rename {0} to {1}!", filepath.filename().string(), newName);
		}
	}

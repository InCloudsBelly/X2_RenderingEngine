...
namespace X2 {
	...
	public:
		...
		{
			X2_CORE_ASSERT(index >= 0 && index < m_Selections.size());
	...
	public:
		static std::mutex s_LockMutex; // ensure only one thread accessing file system content at once
	...
	private:
		...
		std::map<std::string, Ref<VulkanTexture2D>> m_AssetIconMap;

...
[[vk::binding(1)]] Texture2D<float4>           u_ViewNormal;   // source normal map
[[vk::binding(2)]] Texture2D<uint>             u_HilbertLut;   // hilbert lookup table
[[vk::combinedImageSampler]] [[vk::binding(3)]] Texture2D<lpfloat> u_HiZDepth;
[[vk::combinedImageSampler]] [[vk::binding(3)]] SamplerState u_samplerPointClamp;
[[vk::binding(4)]] RWTexture2D<uint>           o_AOwBentNormals;   // output AO term (includes bent normals if enabled - packed as R11G11B10 scaled by AO)
[[vk::binding(5)]] RWTexture2D<unorm float>    o_Edges;   // output depth-based edges used by the denoiser
...
void XeGTAO_MainPass(const int2 outputPixCoord, const int2 inputPixCoords, lpfloat sliceCount, lpfloat stepsPerSlice, const lpfloat2 localNoise)
{
    ...
    float4 deviceZs = u_HiZDepth.GatherRed(u_samplerPointClamp, float2(inputPixCoords * u_ScreenData.InvFullResolution * u_GTAOConsts.X2BUVFactor));
    lpfloat4 valuesUL = (lpfloat4)LinearizeDepth(deviceZs);
    lpfloat4 valuesBR = (lpfloat4)LinearizeDepth(u_HiZDepth.GatherRed(u_samplerPointClamp, float2(inputPixCoords * u_ScreenData.InvFullResolution * u_GTAOConsts.X2BUVFactor), int2(1, 1)));
    ...
    // see "Algorithm 1" in https://www.activision.com/cdn/research/Practical_Real_Time_Strategies_for_Accurate_Indirect_Occlusion_NEW%20VERSION_COLOR.pdf
    {
        ...
        for (lpfloat slice = 0; slice < sliceCount; slice++)
        {
            ...
            for (lpfloat step = 0; step < stepsPerSlice; step++)
            {
                ...
                float  SZ0 = LinearizeDepth(u_HiZDepth.SampleLevel(u_samplerPointClamp, sampleScreenPos0 * u_GTAOConsts.X2BUVFactor, mipLevel).x);
                float3 samplePos0 = XeGTAO_ComputeViewspacePosition(sampleScreenPos0, SZ0);

                float2 sampleScreenPos1 = normalizedScreenPos - sampleOffset;
                float  SZ1 = LinearizeDepth(u_HiZDepth.SampleLevel(u_samplerPointClamp, sampleScreenPos1 * u_GTAOConsts.X2BUVFactor, mipLevel).x);

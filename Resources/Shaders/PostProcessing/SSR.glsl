...
layout(binding = 5) uniform sampler2D u_MetalnessRoughness;
layout(binding = 6) uniform sampler2D u_VisibilityBuffer;

#define GTAO_REFLECTION_OCCLUSION (__X2_REFLECTION_OCCLUSION_METHOD & X2_REFLECTION_OCCLUSION_METHOD_GTAO)
#define HBAO_REFLECTION_OCCLUSION (__X2_REFLECTION_OCCLUSION_METHOD & X2_REFLECTION_OCCLUSION_METHOD_HBAO)

#if GTAO_REFLECTION_OCCLUSION
	layout(binding = 7) uniform usampler2D u_GTAOTex;
#endif

#if HBAO_REFLECTION_OCCLUSION
	layout(binding = 8) uniform sampler2D u_HBAOTex;
#endif
...
{
	return ivec2(textureSize(u_HiZBuffer, mipLevel) * u_SSRInfo.X2BUVFactor);
}
